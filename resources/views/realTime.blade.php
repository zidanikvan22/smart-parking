    ...
    <div class="flex flex-col items-center col-span-2 row-span-3 mt-5">
        @if($selectedZona)
            <button data-modal-target="informasi-zona-{{ $selectedZona->id }}"
                ...
                class="flex items-center justify-center w-1/3 py-3 -mt-3 text-sm text-white bg-blue-500 rounded-lg mt-5">
        ...
        <a href="{{ asset('img/peta.png') }}" download="PetaParkir.jpg"
            class="flex items-center justify-center w-1/2 py-3 -mt-3 text-sm text-blue-700 hover:underline rounded-lg mt-1">
            ...
            <div class="grid grid-cols-4 gap-3 p-5 m-5 bg-white rounded-lg shadow-lg">
                @if($subzona->slot->isNotEmpty())
                    @foreach($subzona->slot as $slot)
                        <div class="{{ $slot->keterangan == 'Terisi'
                                    ? 'bg-red-500'
...
@else
    <div class="p-4 text-center">
        <p class="text-gray-500 font-bold">[Tidak ada Subzona dan slot yang bisa ditampilkan]</p>
    </div>
@endif
